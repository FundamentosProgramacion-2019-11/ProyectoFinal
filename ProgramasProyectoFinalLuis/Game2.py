...
def enemigoD(ventana,x,y):
    global momiaD
    for i in range(1, 19):
        momiaD.append(pygame.transform.scale(pygame.image.load(
            "../Imagenes/Enemigos/" + str(i) + ".png"), [128, 128]))
    global pasoMomia
    ventana.blit(momiaD[pasoMomia], [x, y])
...
def Level2():
    ...
    while not termina:  # Ciclo principal, MIENTRAS la variable termina sea False, el ciclo se repite automáticamente
        ...
        fnivel2 = pygame.image.load('../Imagenes/fondo3.jpg')
        fondoNivel2 = pygame.transform.scale(fnivel2, [ANCHO, ALTO])
        ventana.blit(fondoNivel2, [0, 0])
        ...
        for x in range(cantidadMomias2):
            enemigoD(ventana, (-800 + gap)+int(tiempo)*10, ALTO - 128)
            gap+=32

        for x in range(bossC):
            boss(ventana,ANCHO//2-121,104)
        if atack==False:
            for i in range(10):
        ...
        if (cantidadMomias<=0 and cantidadMomias2 <= 0 and bossC<=0):
            ventana.fill(NEGRO)
            ventana.blit(font.render("WINNER", True, DORADO), [ANCHO // 2 - 100, 100])
            if tiempo>=100:
                termina= True
